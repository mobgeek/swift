...
Resposta:
*/
//  Quando removemos 'default', Swift lembra que o Switch precisa ser exaustivo e, sugere adicionar um default clause. (Erro: Switch must be exhaustive, consider adding a default clause)

//  Todo Switch precisa se exaustivo, ou seja, completo. Isso quer dizer que todos os casos possíveis do tipo em questão precisam ser considerado. E como isso se dá na prática? No exemplo acima default funciona com um caso para todos aqueles casos específicos de legumes que não foram considerados nos "cases" de switch.

//  A posição do default é relevante dentro do Switch. Ele sempre deve aparecer por último.
/*: 
...
Resposta:
*/
func novaSaudação(nome: String, almoçoDoDia: String) -> String {
    
    return "Olá \(nome), hoje servimos \(almoçoDoDia) para o almoço ;-)"
    
}

novaSaudação("Viviane", "\'Feijão Com Arroz\'")
/*:
...
Veja que que podemos usar uma propriedade de `Array` chamada `count` para contar seus elementos retornando o total de elementos que ela possui
*/
func médiaDe(números: Int...) -> Int {
...
enum respostaServidor {
    ...
    case Resultado(String, String)
    case Erro(String)
//:Adicinamos mais um membro a enum: o `Servidor` não responderá, ou seja, não há resposta.
    case Nenhuma
...
case let .Erro(erro):
    ...
    let respostaServidor = "Falha...  \(erro)"
//:Adicionamos mais um caso para considerar o membro `.Nenhuma` que adicionamos a enum `respostaServidor` anteriormente.
case .Nenhuma:
    
    let respostaServidor = "O servidor não respondeu."
