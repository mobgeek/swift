...
println("A biblioteca de midia possui \(contaFilme) filmes e \(contaMúsica) músicas")
...
for item in biblioteca {
    ...
    if let filme = item as? Filme {
        ...
        println("Filme: '\(filme.nome)', dir. \(filme.diretor)'")
        
    } else if let música = item as? Musica {
    
        println("Música: '\(música.nome)', por \(música.artista)'")
    }
}
...
for coisa in coisas {
    ...
    case 0 as Double:
        ...
        println("0 como um Double")
        
    case let umString as String:
        
        println("um string com valor \"\(umString)\"")
        
    case let(x,y) as (Double, Double):
        
        println("um ponto (x,y): (\(x), \(y))")
        
    case let filme as Filme:
        
        println("Um filme chamado '\(filme.nome)' , dir. \(filme.diretor)'")
        
    case let stringConverter as String -> String:
        
        println(stringConverter("Maria"))
